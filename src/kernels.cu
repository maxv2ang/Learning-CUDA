...
__global__ void flash_attn_kernel(
  ...
  // ---------- stage Q tile + zero the accumulators -------------------------
  {
  ...
  for (int s0 = 0; s0 < s_limit; s0 += BC) {
    ...
    if (row_valid) {
      ...
      // ---- dot phase: this lane owns source j ------------------------------
      float dot = -INFINITY;
      if (!masked) {
      ...
      if (m_tile != -INFINITY) {           // skip fully-masked tiles
        ...
        s_p[warp * BC + lane] = p;
        __syncwarp();                        // publish p within this warp
    ...
    __syncthreads();                       // tile state free before next staging
  }

  // ---------- normalize and write the output row ----------------------------
  if (row_valid) {
    const float inv_l = (l > 0.0f) ? (1.0f / l) : 0.0f;
    for (int d = lane; d < H; d += BC)
      o[((long long)(b * tgt_seq + t) * q_heads + qh) * H + d] =
          CudaTypeTraits<T>::from_float(s_acc[warp * H + d] * inv_l);
  }
}
